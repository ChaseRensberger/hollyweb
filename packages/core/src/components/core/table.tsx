...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "group transition-colors hover:bg-muted/50 data-[state=selected]:bg-accent/60 [&>td]:border-b [&>td]:border-border/70",
        className,
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-12 border-b px-4 text-left align-middle text-xs font-medium tracking-wide first:pl-5 last:pr-5 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
