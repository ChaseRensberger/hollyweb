...
import type { HollyDraftDatabase } from "./db";
...
export const hash = (value: string) => new Bun.CryptoHasher("sha256").update(value).digest("hex");

export function returnPath(requested: string, origin: string): string {
  const fallback = "/";
  if (
...
export function createAuth(db: HollyDraftDatabase, config: HollyDraftConfig) {
  const client = createClient({ clientID: config.clientID, issuer: config.authOrigin });
  const secure = config.origin.startsWith("https://");
  const name = secure ? "__Host-hollydraft-session" : "hollydraft-session";
  const flowName = secure ? "__Host-hollydraft-flow" : "hollydraft-flow";
  ...
  router.get("/login", async (c) => {
    ...
    db.insert(loginFlows)
      .values({
        ...
        returnTo: returnPath(c.req.query("returnTo") ?? "/", config.origin),
  ...
  router.get("/callback", async (c) => {
    ...
    if (
      ...
      return c.text("This login link expired or is invalid. Sign in again.", 400);
    }
    const result = await client.exchange(c.req.query("code")!, callback, flow.verifier);
    if (result.err) return c.text("Login failed. Sign in again.", 401);
    const verified = await verifyUser(client, result.tokens.access, config.clientID);
    if (verified.err) return c.text("Login verification failed.", 401);
